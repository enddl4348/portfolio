...
if($_POST['mode'] == "prd_view") {
	...
	// 즐겨찾기
	$sql2 = "SELECT * FROM wiz_wishlist WHERE prdcode = '$prdcode' AND mallid = '$mallid' AND memid = '".$wiz_admin[id]."'";
	$result2 = mysql_query($sql2) or error(mysql_error());
	if($row2 = mysql_fetch_assoc($result2)) $img_src = "/admin/img/ico/wish_on.png";
	else $img_src = "/admin/img/ico/wish.png";
?>
	<div class="prd-vw-btn pop-btn">
		<button type="button" class="view-all"></button>
		<button type="button" class="cls-pop" onclick="scrollOn();"></button>
	</div>

  <script>
  //레이어 팝업 닫았을 때 바디 스크롤 되게
    function scrollOn() {
		...
		<section class="prd-view-top clearfix">
			...
			<div class="prd-buy-sect">
				...
				<div class="cmn__prd-btn">
					<button type="button" class="wish" onclick="prd_view_wish('<?=$row['prdcode']?>', '<?=$row['mallid']?>');"> <img id="wish_img" src="<?=$img_src?>" alt="찜하기 아이콘"> </button>
...
// 즐겨찾기
} else if($_POST['mode'] == "prd_view_wish") {

	$sql = "SELECT * FROM wiz_wishlist WHERE prdcode = '$prdcode' AND mallid = '$mallid' AND memid = '".$wiz_admin[id]."'";
	$result	= mysql_query($sql) or error(mysql_error());

	if($row = mysql_fetch_assoc($result)) {
		$sql = "DELETE FROM wiz_wishlist WHERE prdcode = '$prdcode' AND mallid = '$mallid' AND memid = '".$wiz_admin[id]."'";
		mysql_query($sql) or error(mysql_error());
		echo 0;
	} else {
		$sql = "INSERT INTO wiz_wishlist SET
					prdcode = '$prdcode',
					mallid = '$mallid',
					memid = '".$wiz_admin[id]."',
					wdate = now()";
...
// 선택 상품 주문
	...
	for($i=0; $i<count($sel_prdcode); $i++) {
		...
		$bt_result	= mysql_query($bt_sql) or error(mysql_error());
		if($bt_row = mysql_fetch_assoc($bt_result)) {
			$sql = "UPDATE wiz_basket_tmp SET amount = '".$amount."' WHERE idx = '".$row['idx']."'";
			mysql_query($sql) or error(mysql_error());
		} else {
...
?>
				...
				<div class="buy-list-item">
					<table class="cart__list">
						...
						while($cart_row = mysql_fetch_assoc($cart_result)) {
							...
							$psql = "SELECT * FROM wiz_product WHERE prdcode = '".$cart_row['prdcode']."'";
							$presult = mysql_query($psql) or error(mysql_error());
							$prow = mysql_fetch_assoc($presult);

							$min_date = date("Y-m-d", strtotime("+".$prow['min_date']." days"));
						...
						?>
							...
							<tr class="req_date">
								<td colspan="4">  
									<dl class="clearfix">
										<dt>납품 요청일</dt>
										<dd> <input type="text" name="del_date[<?=$cart_row['idx']?>]" value="<?=$min_date?>" class="prd__datepicker" autocomplete="off"></dd>
									</dl>
								</td>
							</tr>
						<? } ?>
		...
		<section class="buy-prc-sect cmn-prd-btn">
			<div class="dvey-info">
				...
				<dl class="delv-sect">
					<dt>배송지 선택</dt>
					<dd>
						<select onchange="sel_del(this.value);">
						<option value="">선택</option>
						<?
						$add_sql = "SELECT * FROM wiz_address WHERE id = '".$wiz_admin['id']."' ORDER BY wdate DESC";
						$add_result = mysql_query($add_sql) or error(mysql_error());
						while($add_row = mysql_fetch_assoc($add_result)) {
						?>
							<option value="<?=$add_row['name']?>^<?=$add_row['hphone']?>^<?=$add_row['post']?>^<?=$add_row['address']?>^<?=$add_row['address2']?>"><?=$add_row['del_name']?></option>
						<? } ?>
						</select>
					</dd>
					<dt>수령자</dt>
			...
			<div class="prd-sect">
				...
				<button type="button" class="req-btn" onclick="order_save();"><?=$buy_text?></button>
				<? if($wiz_admin['approval_buy'] == "Y") { ?>
				<?
				...
				$payment1 = explode("^", $de_row['payment1']);
				$payment2 = explode("^", $de_row['payment2']);
				$payment3 = explode("^", $de_row['payment3']);

				$app_mem_sql = "SELECT * FROM wiz_member WHERE depart_code = '".$payment1[0]."' AND level = '388'";
				$app_mem_result = mysql_query($app_mem_sql) or error(mysql_error());
				$app_mem_row = mysql_fetch_assoc($app_mem_result);
				array_push($payment1, $app_mem_row['name']);
				array_push($payment1, $app_mem_row['id']);

				$app_mem_sql = "SELECT * FROM wiz_member WHERE depart_code = '".$payment2[0]."' AND level = '388'";
				$app_mem_result = mysql_query($app_mem_sql) or error(mysql_error());
				$app_mem_row = mysql_fetch_assoc($app_mem_result);
				array_push($payment2, $app_mem_row['name']);
				array_push($payment2, $app_mem_row['id']);

				$app_mem_sql = "SELECT * FROM wiz_member WHERE depart_code = '".$payment3[0]."' AND level = '388'";
				$app_mem_result = mysql_query($app_mem_sql) or error(mysql_error());
				$app_mem_row = mysql_fetch_assoc($app_mem_result);
				array_push($payment3, $app_mem_row['name']);
				array_push($payment3, $app_mem_row['id']);

				$app_no = 1;
				?>
				<div class="rep-step__wrap clearfix">
					<ul class="rep-step__box">
						<li>요청자</li>
						<li><?=$de_row['sub_name']?></li>
						<li><?=$wiz_admin['name']?></li>
					</ul>
					<? for($i=1; $i<=3; $i++) { ?>
						<? if(${"payment".$i}[1] != "") { ?>
							<ul class="rep-step__box">
								<input type="hidden" name="payment<?=$app_no?>" value="<?=${"payment".$i}[3]?>">
								<li>승인(<?=$app_no?>차)</li>
								<li><?=${"payment".$i}[1]?></li>
								<li><?=${"payment".$i}[2]?></li>
							</ul>
				...
				<!--p class="apprv-btn-wrap"><button type="button" class="apprv-btn">결재선 변경</button></p-->
				<? } ?>
			</div>
    ...
    // datepicker 오늘 날짜 배경색 관련코드 여기까지
	...
	$(".prd__datepicker").datepicker({
		...
		// 오늘날짜와 같으면 오늘 날짜 색깔 변경 관련 코드 여기까지
	}); //datepicker
	</script>
<?
...
// 구매하기
	...
	if($de_row['use_budget'] == "N") {
		$data->status = "N";
		$data->message = "예산설정이 미사용입니다.";
		echo json_encode($data);
		exit;
	} else {

		// 구매전 남은예산
		$budget = purchase_budget($wiz_admin['depart_code']);

		if($de_row['approval_buy'] == "N" && $de_row['approval_over_budget'] == "N") {

			if($budget <= $total_price) {
				$data->status = "N";
				$data->message = "예산이 부족합니다.";
				echo json_encode($data);
				exit;
			} else {
				$message = "구매되었습니다.";
		...
		} else if($de_row['approval_buy'] == "N" && $de_row['approval_over_budget'] == "Y") {

			if($budget <= $total_price) {
				$message = "예산 초과 구매승인요청 되었습니다.";
				$status = "AR";
			} else {
				$message = "구매되었습니다.";
				$status = "OD";
		...
		foreach($idx_arr as $idx) {
			...
			$sql = "INSERT INTO wiz_basket SET
						sid = '".$SID."',
						tmp_idx = '".$row['idx']."',
						orderid = '".$orderid."',
						prdcode = '".$row['prdcode']."',
						...
						del_date = '".$del_date[$row['idx']]."',
						com_request = '".$com_request[$row['mallid']]."'";
			mysql_query($sql) or error(mysql_error());

			$sql = "DELETE FROM wiz_basket_tmp WHERE idx = '$idx' AND uniq_id = '".$wiz_admin['id']."'";
			mysql_query($sql) or error(mysql_error());
		}

		$sql = "INSERT INTO wiz_order SET
					sid = '".$SID."',
					orderid = '".$orderid."',
					depart_code = '".$wiz_admin['depart_code']."',
					approval_buy = '".$wiz_admin['approval_buy']."',
					send_id = '".$wiz_admin['id']."',
					send_name = '".$wiz_admin['name']."',
					send_hphone = '".$hphone."',
					send_email = '".$wiz_admin['email']."',
					send_post = '".$post."',
					send_address = '".$address."',
					send_address2 = '".$address2."',
					rece_name = '".$name."',
