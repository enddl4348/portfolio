		...
		<li <?if($dep_code == "") echo "class='on'";?>>
			...
			?>
				<div class="depth-menu-box">
					<strong class="nav__type-tit"><a href="purc_req.php?dep_code=<?=$row->catcode?>"><?=$row->catname?></a></strong>
					<ul class="nav__depth1-list">
					<?
					$sql2 = "select substring(catcode,3,2) as catcode, catname from wiz_category where depthno = 2 and catcode like '".$row->catcode."%' order by priorno02 asc";
					$result2 = mysql_query($sql2) or error(mysql_error());
					while($row2 = mysql_fetch_object($result2)){
					?>
						<li><a href="purc_req.php?dep_code=<?=$row->catcode?>&dep2_code=<?=$row2->catcode?>"><?=$row2->catname?></a></li>
					<? } ?>
					</ul>
				</div>
			<?
...
<form name="searchForm" action="<?=$PHP_SELF?>" method="get">
	...
	<div class="prd-opt-wrap">
		<a href="purc_etc.php"><span class="home ir_pm">홈</span></a>
		<em class="arr-ico"></em>
...
<div class="purc__prd-list cmn-prd-list">
	...
	$sql = "SELECT wp.*, wpm.memprice, wpm.mallid,
			...
			FROM wiz_product wp
				...
				INNER JOIN wiz_wishlist ws ON wp.prdcode = ws.prdcode AND wm.id = ws.mallid
			WHERE $depart_sql $catcode_sql $search_sql wp.dell_check != 'DELL' AND ws.memid = '".$wiz_admin['id']."'";
	$result	= mysql_query($sql) or error(mysql_error());
	...
	<div class="prd-list-type">
		...
		?>
			<div class="list-clm-item">
				...
				<div class="prd-desc">
					...
					<!-- 여기 수정된곳 -->
					<div class="prd-opt">
						<p class="row-wrap"><span class="prd-model">모델명: <em><?=$row->model_name?></em> </span> <span>사양: <em><?=$row->spec?></em> </span></p>
						<p class="row-wrap"><span>포장단위: <em><?=$row->packing?> <?=$row->packing_unit?>/<?=$row->packing_unit2?></em></span> <span>주문단위: <em><?=$row->order_unit?></em> </span> </p>
						<span class="last">공급사: <em><?=$row->com_name?></em> </span>
						<!-- 원래 있던 거 -->
						<!-- <span><?=$row->maker?> 자생</span>
						<span><?=$row->spec?> sp</span>
						<span><?=$row->order_unit?> BOX</span> -->
					</div>
				</div> <!-- //prd-desc -->
			</div>
		<? } ?>
		...
		?>
			<tr>
				...
				<td class="prd-desc row-desc">
					<strong class="prd-name"><?=$row->prdname?></strong>
					<div class="prd-opt">
						<!-- 여기 추가된 부분 -->
						<p class="dt-wrap"><span>상품코드: <em><?=$row->prdcode?></em> </span> <span>모델명: <em><?=$row->model_name?></em> </span> <span>사양: <em><?=$row->spec?></em></span></p>
						<p class="dt-wrap"><span>포장단위: <em><?=$row->packing?> <?=$row->packing_unit?>/<?=$row->packing_unit2?></em></span> <span>주문단위: <em><?=$row->order_unit?></em></span></p>
						<p class="dt-wrap"><span>최소주문수량: <em><?=$row->min_amount?></em></span> <span>최소납기일: <em><?=$row->min_date?>일</em> </span></p>
						<!-- 원래있던 애들 -->
						<!-- <span><?=$row->maker?></span> <span><?=$row->spec?></span> <span><?=$row->order_unit?></span> -->
					</div>
				</td>

				<td class="prd-prc" width="385px">
          <strong class="producer"><?=$row->com_name?></strong> <!-- 여기도 추가 -->
          <em><?=number_format($row->memprice)?></em> 원
        </td>
